...
xvals = np.linspace(0.0, Lx-10**(-10), 5*nx)
...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    psi_exact_exprH0 = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu*((H0+eta_exact_expr)/H0)**2*(GX+GY))-(u0*x[0])
    psi_exact_exprz = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu*((x[2])/H0)**2*(GX+GY))-(u0z*x[0])
    sickofit =         ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu*((H0+eta_exact_expr)/H0)**2*(GX+GY))-(u0*x[0])
    
    btopoexpr = 0.0*psi_exact_exprH0 # no topography
    ttilde = 5 # time units used for SP2 in BLE
    t_end = ttilde*(H0/np.sqrt(gg*H0*muu)) # dimensionless BLE end-time of BokhoveKalogirou2016 times time-scaling factor
    ...
    ttilde = 0.005 # BLE time units used for SP2 in BLE
    t_end = ttilde*(H0/np.sqrt(gg*H0*muu)) # dimensionless BLE end-time of BokhoveKalogirou2016 times time-scaling factor
    ...
    while (t <= t_end+dt):
        ...
        t+= dt
    nplot = 2
    # PETSc.Sys.Print print('Hallo SP3.')
    tijd.sleep(0.1)
# End if
...
if nvpcase=="MMP": # modfied midpoint for solving psimp, hmp, varphimp= fd.split(result_mixedmp)
    ...
    if nfullgrav == 0:
        VP3dpf = (- H0*Ww*fd.inner(psimp, (h_new - h_old)/dt) \
                  ...
                  + H0*gg*Ww*( 0.5*fd.inner(fac*H0+hmp, fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) ) * fd.ds_t(degree=vpoly) \
                  + 0.5*( (Lw**2/Ww)*(fac*H0+hmp)*(u0z + psimp.dx(0)*phihat+varphimp.dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                          + Ww*(fac*H0+hmp)*(u0zpy + psimp.dx(1)*phihat+varphimp.dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                          + Ww*(H0**2/(fac*H0+hmp)) * (u0zpz + psimp*dphihat+varphimp.dx(2))**2 ) * fd.dx(degree=vpoly) 
        #   - H0*gg*Ww*( 0.5*fd.inner(fac*H0+hmp, fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) * fd.ds_b(degree=vpoly)
    else:
        VP3dpf = (- H0*Ww*fd.inner(psimp, (h_new - h_old)/dt) \
                  + H0*fd.inner(hmp, (Ww*psii - Ww*psi_f)/dt) ) * fd.ds_t(degree=vpoly) \
                  + ( 0.5*( (Lw**2/Ww)*(fac*H0+hmp)*(u0z + psimp.dx(0)*phihat+varphimp.dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                          + Ww*(fac*H0+hmp)*(u0zpy + psimp.dx(1)*phihat+varphimp.dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                            + Ww*(H0**2/(fac*H0+hmp)) * (u0zpz + psimp*dphihat+varphimp.dx(2))**2 ) \
                      + gg*Ww*H0*(fac+hmp/H0)*(x[2]*(fac*H0+hmp)/H0-H0) ) * fd.dx(degree=vpoly)
    ...
    phi_combonl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexprnl, result_mixedmp, bcs = BC_varphi_mixedmp), solver_parameters=param_psi4555m)
    ...
    VP3dpf = (- H0*Ww*fd.inner(psisv, (h_new - h_old)/dt) + H0*Ww*fd.inner(psii, h_new/dt) - H0*Ww*fd.inner(psi_f, h_old/dt) \
              ...
              + 0.5*H0*gg*Ww*( 0.5*fd.inner(fac*H0+h_old, fac*H0+h_old)-(fac*H0+h_old)*H0+0.5*H0**2 )  ) * fd.ds_t \
                + 0.25*( (Lw**2/Ww)*(fac*H0+h_new)*(u0z+psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(0)+x[2]*h_new.dx(0))*(u0zpz + psisv*dphihat+varphisv.dx(2)))**2 \
                        + Ww*(fac*H0+h_new)*(u0zpy + psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(1)+x[2]*h_new.dx(1))*(u0zpz + psisv*dphihat+varphisv.dx(2)))**2 \
                        + Ww*(H0**2/(fac*H0+h_new)) * (u0zpz+psisv*dphihat+varphisv.dx(2))**2 \
                        + (Lw**2/Ww)*(fac*H0+h_old)*(u0z+psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(0)+x[2]*h_old.dx(0))*(u0zpz + psisv*dphihat+varphisv.dx(2)))**2 \
                        + Ww*(fac*H0+h_old)*(u0zpy+psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(1)+x[2]*h_old.dx(1))*(u0zpz + psisv*dphihat+varphisv.dx(2)))**2 \
                        + Ww*(H0**2/(fac*H0+h_old)) * (u0zpz +psisv*dphihat+varphisv.dx(2))**2  ) * fd.dx
...
if nic=="SP3" and ntflag==0 and ntflag2==0:
    outfile_psi = fd.File("resultsntflag0/psi.pvd")
    outfile_height = fd.File("resultsntflag0/height.pvd")
    outfile_varphi = fd.File("resultsntflag0/varphi.pvd")
    outfile_sicks = fd.File("resultsntflag0/sicks.pvd")
    outfile_sickss = fd.File("resultsntflag0/sickss.pvd")
    fileE = "resultsntflag0/potflow3dperenergy.txt"
    # PETSc.Sys.Print
    print('ntflag=, ntflag2=',ntflag,ntflag2)
elif nic=="SP3" and ntflag==1 and ntflag2==0:
    outfile_psi = fd.File("resultsntflag1/psi.pvd")
    outfile_height = fd.File("resultsntflag1/height.pvd")
    outfile_varphi = fd.File("resultsntflag1/varphi.pvd")
    outfile_sicks = fd.File("resultsntflag1/sicks.pvd")
    outfile_sickss = fd.File("resultsntflag1/sickss.pvd")
    fileE = "resultsntflag1/potflow3dperenergy.txt"
    # PETSc.Sys.Print
    print('ntflag=, ntflag2=',ntflag,ntflag2)
else:
    outfile_psi = fd.File("results2/psi.pvd")
    outfile_height = fd.File("results2/height.pvd")
    outfile_varphi = fd.File("results2/varphi.pvd")
    outfile_sicks = fd.File("results2/sicks.pvd")
    outfile_sickss = fd.File("results2/sickss.pvd")
    fileE = "results2/potflow3dperenergy.txt"
...
while t <= 1.0*(t_end + dt): #  t_end + dt
    ...
    if t>=0.0:
        if nvpcase=="MMP":
            EKin = fd.assemble( 0.5*( (Lw**2/Ww)*(fac*H0+hmp)*(u0z + psimp.dx(0)*phihat+varphimp.dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                                      + Ww*(fac*H0+hmp)*(u0zpy + psimp.dx(1)*phihat+varphimp.dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                                      + Ww*(H0**2/(fac*H0+hmp))*(u0zpz +psimp*dphihat+varphimp.dx(2))**2 ) * fd.dx(degree=vpoly) )
            if nfullgrav == 0:
                EPot = fd.assemble( H0*gg*Ww*( 0.5*fd.inner(fac*H0+hmp,fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) * fd.ds_t(degree=vpoly) ) \
                #  - H0*gg*Ww*( 0.5*fd.inner(fac*H0+hmp, fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) * fd.ds_b(degree=vpoly) )
            else:
                EPot = fd.assemble( gg*Ww*H0*(fac+hmp/H0)*(x[2]*(fac*H0+hmp)/H0-H0) * fd.dx(degree=vpoly) )
        elif nvpcase=="SV":
            EKin = fd.assemble( 0.25*( (Lw**2/Ww)*(fac*H0+h_new)*(u0z + psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(0)+x[2]*h_new.dx(0))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + Ww*(fac*H0+h_new)*(u0zpy + psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(1)+x[2]*h_new.dx(1))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + Ww*(H0**2/(fac*H0+h_new)) * (u0zpz + psisv*dphihat+varphisv.dx(2))**2 \
                                       + (Lw**2/Ww)*(fac*H0+h_old)*(u0z + psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(0)+x[2]*h_old.dx(0))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + Ww*(fac*H0+h_old)*(u0zpy + psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(1)+x[2]*h_old.dx(1))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + Ww*(H0**2/(fac*H0+h_old)) * (u0zpz + psisv*dphihat+varphisv.dx(2))**2  ) * fd.dx  )
