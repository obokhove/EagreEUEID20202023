...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    u0 = (psiLx_exact_exprH0-psiqq0_exact_exprH0)/Lx 
...
if nvpcase=="MMP": # modfied midpoint for solving psimp, hmp, varphimp= fd.split(result_mixedmp)
    ...
    param_psi45 = {'ksp_converged_reason':None, 'ksp_type': 'gmres', 'snes_type': 'ksponly', 'pc_type': 'lu'} # 1 iteration same _psi1; at A=1 (too high back at A=0.2), CFL=2 collapses but gets further than 46; 45 at A=0.2 and cfl=2 gives energy decay; 0.6m waves on 1m depth; best one?
    ...
    phi_combonl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexprnl, result_mixedmp, bcs = BC_varphi_mixedmp), solver_parameters=param_psi45)
    ...
    param_psi45 = {'ksp_converged_reason':None, 'ksp_type': 'gmres', 'snes_type': 'ksponly', 'pc_type': 'lu'} # 1 iteration; best one?
