...
if nic=="SP1": # soliton decaying sufficiently fast to be taken as periodi using BL expansion in z
    ...
    x2hat = (np.sqrt(muu/eps)*(np.sqrt(2)/3)**(1/3)*X2KPE+(1/eps)*np.sqrt(0.5*muu/eps)*tau0) # KPE X-scale to BLE xhat-scale
    x1hat = (np.sqrt(muu/eps)*(np.sqrt(2)/3)**(1/3)*X1KPE+(1/eps)*np.sqrt(0.5*muu/eps)*tau0) # KPE X-scale to BLE xhat-scale
...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    eta_exact_expr = 2.0*eps*H0*((4/3)**(1/3))*( KKXX/KK -(KKX/KK)**2 )
    psi_exact_exprH0 = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu*((H0+0.0*etax2y)/H0)**2*(GX+GY))-(u0*x[0])
    psi_exact_exprz = ((eps*H0*np.sqrt(gg*H0))/np.sqrt(muu))*np.sqrt(eps)*(4*np.sqrt(2)/9)**(1/3)*(2*KKX/KK-0.5*muu*((x[2])/H0)**2*(GX+GY))-(u0z*x[0])
