...
nsavelocalHPC = 0 # Save files locally or on HPC in nobackup; directories specified; sets "save_path"; 0 is local
nvals = 0 #  no figure used when on HPC or when silly xvals does not work
nprintout = 1 # print out on screen or in file; 1: on screen: 0: PETSC version
...
if nic=="SP1": # soliton decaying sufficiently fast to be taken as periodi using BL expansion in z
    ...
    Lz = 20.0
    H0 = Lz
    that0 = -100 # BLE time all seems to work for that0=0 but not for that0=-200 JUNHO: please check.
    ntflag = 1
    ...
    Atilde = 0.5*(tantheta/(2+deltaa*np.sqrt(2)))**2 # Expression (26b) in WW-paper
    ...
    value = ((2/9)**(1/6)*0.25/(0.87013))**2
    if nprintout == 1:
        print('Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE: ',Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE) # printing to compare wth WW-paper data
        print('Ystar,k4,log(2.7),xx1,xx2: ',Ystar,k4,value,np.log(2.7),xx1,xx2) # printing to compare wth WW-paper data
    else:
        PETSc.Sys.Print('Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE: ',Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE) # printing to compare wth WW-paper data
...
if nsiii==0:
    ...
    y0 = sfac
    z0 = 0.0
    xvals = np.linspace(0.0+sfac, Lx-sfac, 5*nx)
    yvals = np.linspace(0.0+sfac, Ly-sfac, ny)
    zvals = np.linspace(0.0+sfac, Lz-sfac, nz) #
    # print('yvals',yvals)
    xslice = 0.5*Lx
    yslice = nfacc*sfac
    yslice12 = 0.5*Ly
    yslice10 = Ly-nfacc*sfac
    zslice = H0-nfacc*sfac
else: # 27-03-2023 one for SP2
    xvals = np.linspace(0.0+10**(-10), Lx-10**(-10), nx)
    yvals = np.linspace(0.0+10**(-10), Ly-10**(-10), ny)
    zvals = np.linspace(0.0+10**(-10), Lz-10**(-10), nz)
    zslice = H0-10**(-10)
    xslice = 0.5*Lx
    yslice = 0.0+10**(-10)
    yslice12 = 0.5*Ly
...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    ttilde = 0.5 # BLE final time units used for SP3 in BLE
...
t_plot = time[0::nt]
if nprintout==1:
    print(' S: dtmeet tmeet tmeas', dtmeet, tmeet, tmeas)
    print('t_plot', t_plot,nt,nplot, t_end)
else:
    PETSc.Sys.Print(' S: dtmeet tmeet tmeas', dtmeet, tmeet, tmeas)
    PETSc.Sys.Print('t_plot', t_plot,nt,nplot, t_end)
...
if nvpcase=="MMP": # modfied midpoint for solving psimp, hmp, varphimp= fd.split(result_mixedmp)
    ...
    # 'snes_monitor': None,'snes_view': None, 'ksp_monitor_true_residual': None, 'snes_converged_reason': None, 'ksp_converged_reason': None
    # https://www.firedrakeproject.org/preconditioning.html h-multigrid? codes available
    # https://fenics2021.com/talks/brubeck.html p multigrid https://arxiv.org/pdf/2107.14758.pdf no codes available
    ...
    param_psi4555 = {'snes_atol': 1e-14, 'ksp_converged_reason':None, "ksp_rtol": 1e-14, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
                     'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'ilu',\
                     'fieldsplit_2_ksp_type': 'gmres', 'fieldsplit_2_pc_type': 'gamg'}
    param_psi4555m = {'snes_atol': 1e-12, 'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
                      'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'ilu',\
                      'fieldsplit_2_ksp_type': 'gmres', 'fieldsplit_2_pc_type': 'gamg','pc_factor_mat_solver_type':'mumps','snes_monitor':None, 'ksp_monitor':None}
    param_psi4555cg = {'snes_atol': 1e-12, 'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
                      'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'ilu',\
                      'fieldsplit_2_ksp_type': 'cg', 'fieldsplit_2_pc_type': 'gamg','pc_factor_mat_solver_type':'mumps','snes_monitor':None, 'ksp_monitor':None}
    param_psi4555cgg = {'snes_atol': 1e-12, 'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'gamg',\
                      'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'gamg',\
                      'fieldsplit_2_ksp_type': 'cg', 'fieldsplit_2_pc_type': 'gamg','pc_factor_mat_solver_type':'mumps','snes_monitor':None, 'ksp_monitor':None}
    param_psi4555cgh = {'snes_atol': 1e-12, 'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
                        'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'ilu',\
                        'fieldsplit_2_ksp_type': 'cg', 'fieldsplit_2_pc_type': 'hypre','pc_factor_mat_solver_type':'mumps','snes_monitor':None, 'ksp_monitor':None}
    param_psi4555cgm = {'snes_atol': 1e-12, 'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
                        'fieldsplit_1_ksp_type': 'gmres', 'fieldsplit_1_pc_type': 'ilu',\
                        'fieldsplit_2_mat_type': 'matfree','fieldsplit_2_ksp_type': 'cg', 'fieldsplit_2_pc_type': 'python', 'pc_python_type': 'firedrake.AssembledPC','pc_factor_mat_solver_type':'mumps','snes_monitor':None, 'ksp_monitor':None}
    param_hat_psi = {'ksp_converged_reason':None, 'ksp_type': 'preonly', 'pc_type': 'lu'}
    param_psi = {"ksp_rtol": 1e-8, 'ksp_type': 'preonly', 'pc_type': 'lu'}
    lines_parameters = {'ksp_type': 'gmres', 'ksp_monitor': None, 'ksp_converged_reason': None,
                        'pc_type': 'python', 'pc_python_type': 'firedrake.ASMStarPC', 'star_construct_dim': 2,
    ...
    param_psi4 = {'ksp_converged_reason':None, 'ksp_type': 'gmres', 'pc_type': 'ilu'} # 1 iteration
    ...
    lines_parameters2 = {'ksp_type': 'gmres', 'ksp_monitor': None, 'ksp_converged_reason': None,
                         'pc_type': 'python', 'pc_python_type': 'firedrake.ASMStarPC',
                         'snes_lag_preconditioner_persists': None, 'snes_lag_preconditioner': 5,
    ...
    phi_combonlsv = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexprnl, result_mixedsv, bcs = BC_varphi_mixedsv), solver_parameters=line_parameters2)
    
    #  Step-3: solve h_new=h^(n+1/2) variation wrt psi^(n+1/2)
    h_exprnl1 = fd.derivative(VP3dpf, psisv, du=v_R)
    h_exprnl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(h_exprnl1,h_new), solver_parameters=line_parameters2)

    #  Step-4: variation wrt h_new fsolve psii=psi^(n+1)
    psin_exprnl1 = fd.derivative(VP3dpf, h_new, du=v_R)
    phin_exprnl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(psin_exprnl1 ,psii), solver_parameters=param_psi4)
...
if nsavelocalHPC==1: # save_path =  "/nobackup/$USER/lin_pot_flow/"
    save_path =  "/nobackup/amtob/potflow/"
else:
...
if nic=="SP3" and ntflag==0 and ntflag2==0:
    ...
    outfile_psi = fd.File(save_path+"resultsntflag1/psi.pvd")
    outfile_height = fd.File(save_path+"resultsntflag1/height.pvd")
    outfile_varphi = fd.File(save_path+"resultsntflag1/varphi.pvd")
    outfile_sicks = fd.File(save_path+"resultsntflag1/sicks.pvd")
    outfile_sickss = fd.File(save_path+"resultsntflag1/sickss.pvd")
    fileE = save_path+"resultsntflag1/potflow3dperenergy.txt"
    filEtaMax = save_path+"resultsntflag1/potflow3dperetamax.txt"
    if nprintout==1:
        print('ntflag, ntflag2=',ntflag,ntflag2)
    else:
        PETSc.Sys.Print('ntflag, ntflag2=',ntflag,ntflag2)
elif nic=="SP2":
    outfile_psi = fd.File(save_path+"results3/psi.pvd")
    outfile_height = fd.File(save_path+"results3/height.pvd")
    outfile_varphi = fd.File(save_path+"results3/varphi.pvd")
    outfile_sicks = fd.File(save_path+"results3/sicks.pvd")
    outfile_sickss = fd.File(save_path+"results3/sickss.pvd")
    fileE = save_path+"results3/potflow3dperenergy.txt"
    filEtaMax = save_path+"results3/potflow3dperetamax.txt"
else:
...
if nprintout==1:
    ...
    PETSc.Sys.Print('Plotting starts, initial data:')

if nvals==1:
    plt.figure(1)
    eta1vals = np.array([h_old.at(x, yslice, zslice) for x in xvals]) #
    eta1vals12 = np.array([h_old.at(x, yslice12, zslice) for x in xvals]) #
    eta1vals10 = np.array([h_old.at(x, yslice10, zslice) for x in xvals]) # 
    ...
    ax1.plot(xvals, eta1vals12, '-k', label = f' $\eta_n: t = {t:.3f}$',linewidth=2)
    ax1.plot(xvals, eta1vals10, '-k', label = f' $\eta_n: t = {t:.3f}$',linewidth=2)
...
while t <= 1.0*(t_end + dt): #  t_end + dt
    ...
    if t>=0.0:
        if nvpcase=="MMP":
            EKin = fd.assemble( 0.5*( LwdWw2*(fac*H0+hmp)*(u0z + psimp.dx(0)*phihat+varphimp.dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                                      + (fac*H0+hmp)*(u0zpy + psimp.dx(1)*phihat+varphimp.dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + psimp*dphihat+varphimp.dx(2)))**2 \
                                      + (H0**2/(fac*H0+hmp))*(u0zpz +psimp*dphihat+varphimp.dx(2))**2 ) * fd.dx(degree=vpoly) )
            if nfullgrav == 0:
                EPot = fd.assemble( H0*gg*( 0.5*fd.inner(fac*H0+hmp,fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) * fd.ds_t(degree=vpoly) ) \
                #  - H0*gg*Ww*( 0.5*fd.inner(fac*H0+hmp, fac*H0+hmp)-(fac*H0+hmp)*H0+0.5*H0**2 ) * fd.ds_b(degree=vpoly) )
            else:
                EPot = fd.assemble( gg*H0*(fac+hmp/H0)*(x[2]*(fac*H0+hmp)/H0-H0) * fd.dx(degree=vpoly) )
        elif nvpcase=="SV":
            EKin = fd.assemble( 0.25*( LwdWw2*(fac*H0+h_new)*(u0z + psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(0)+x[2]*h_new.dx(0))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + (fac*H0+h_new)*(u0zpy + psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_new))*(H0*btopo.dx(1)+x[2]*h_new.dx(1))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + (H0**2/(fac*H0+h_new)) * (u0zpz + psisv*dphihat+varphisv.dx(2))**2 \
                                       + LwdWw2*(fac*H0+h_old)*(u0z + psisv.dx(0)*phihat+varphisv.dx(0)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(0)+x[2]*h_old.dx(0))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + (fac*H0+h_old)*(u0zpy + psisv.dx(1)*phihat+varphisv.dx(1)-(1.0/(fac*H0+h_old))*(H0*btopo.dx(1)+x[2]*h_old.dx(1))*(psisv*dphihat+varphisv.dx(2)))**2 \
                                       + (H0**2/(fac*H0+h_old)) * (u0zpz + psisv*dphihat+varphisv.dx(2))**2  ) * fd.dx  )
            EPot = fd.assemble( (0.5*H0*gg*( 0.5*fd.inner(fac*H0+h_new, fac*H0+h_new)-(fac*H0+h_new)*H0+0.5*H0**2 ) \
                                + 0.5*H0*gg*( 0.5*fd.inner(fac*H0+h_old, fac*H0+h_old)-(fac*H0+h_old)*H0+0.5*H0**2 ) ) * fd.ds_t )
        if nE0==0:
            E0 = EKin+EPot
            nE0 = 1
        ...
        if nenergyplot==1:
            ...
            #PETSc.Sys.Print("%.11f %.11f %.11f %.11f" % (t, EPot-E0, EKin, Etot),file=outputE)
            print("%.11f %.11f %.11f %.11f" % (t, EPot-E0, EKin, Etot),file=outputE)
            
    if nvpcase == "SV": # VP SV
        h_old.assign(h_new)
    if t>=0.0:
        if nic=="SP1" or nic=="SP2" or nic=="SP3":
            with h_old.dat.vec_ro as vetamax:
               wtamax = vetamax.max()[1]
            print("%.11f %.11f %.11f" % (t, wtamax, Atilde),file=outputEtamax)
    ...
    if (t in t_plot): # if (t >= tmeet-0.5*dt): # t > tmeet-epsmeet
        if nprintout==1:
            ...
            PETSc.Sys.Print('Plotting starts')   
        plt.figure(1)
        i += 1
        tmeet = tmeet+dtmeet
        ...
        if nvpcase == "MMP" or "SV": #
            if nvals==1:
                ...
                if nic == "linearw":
                    ...
                    ax1.plot(xvals, eta1vals, color[int(i-1) % 4], label = f' $\eta_n: t = {t:.3f}$')
                    ax2.plot(xvals, phi1vals, color[int(i-1) % 4], label = f' $\phi_n: t = {t:.3f}$')
                    ax1.plot(xvals, eta1vals12, color[int(i-1) % 4], label = f' $\eta_n: t = {t:.3f}$')
                    ax2.plot(xvals, phi1vals12, color[int(i-1) % 4], label = f' $\phi_n: t = {t:.3f}$')
                    ax1.plot(xvals, eta1vals10, color[int(i-1) % 4], label = f' $\eta_n: t = {t:.3f}$')
                    ax2.plot(xvals, phi1vals10, color[int(i-1) % 4], label = f' $\phi_n: t = {t:.3f}$')
            if nprintout==1:
                print('t =', t, tmeet, i)
            else:
                PETSc.Sys.Print('t =', t, tmeet, i)                
            #
            outfile_height.write(h_old, time=t)
            outfile_psi.write(psi_f, time=t)
...
if nprintout==1:
    ...
    PETSc.Sys.Print('t=',t,'tmeet=',tmeet,'tplot',t_plot)
    
outputE.close()
outputEtamax.close()

if nsavelocalHPC==1:
    plt.savefig(save_path+'results/figures.png')
else:
    if nvals==1:
        plt.show()
