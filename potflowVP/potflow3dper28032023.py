...
if nic=="SP1": # soliton decaying sufficiently fast to be taken as periodi using BL expansion in z
    ...
    #
    Lyhat = 47
    Ystar = 0.5*np.log(A235/A136)/(k6**2-k5**2)
    y1hat = Ystar*np.sqrt(muu)*(np.sqrt(2)/3)**(2/3)/eps
    y2hat = y1hat+Lyhat
    Y2KPE = (eps/np.sqrt(muu))*(3/np.sqrt(2))**(2/3)*y2hat
    Ly = (H0/np.sqrt(muu))*(y2hat-y1hat)
    yy2 = (H0/np.sqrt(muu))*y2hat
    ...
    Lx = xx2-xx1
    value = ((2/9)**(1/6)*0.25/(0.87013))**2
    print('Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE: ',Lx,Ly,x1hat,x2hat,y1hat,y2hat,X1KPE, X2KPE)
    print('Ystar,k4,log(2.7),xx1,xx2: ',Ystar,k4,value,np.log(2.7),xx1,xx2)
    nx = 132
    ny = 480
    nz = 4
    nCG = 2 # function space order horizontal
