...
vvmpc2, vvmpc0, vvmpc1 = fd.split(vvmpc) # These represent "blocks".
# Q12, Z12 = fd.split(result_mixedmpc) # Only variations needed
Z12, W12, Ic12 = fd.split(result_mixedmpc) # But for completeness these enter the VP
...
if nvpcase==2: # MMP 1-coil nonlinear case
    ...
    if noption==4:
        ...
        bink = Zbar + alp*Hm
        xk = Z12
        KZ12 = 0.564189583547756*fd.sqrt(pi)*(L/2-bink+xk)/(a_rad**3.0*fd.sqrt(1+(L/2-bink+xk)**2/a_rad**2)) - 0.564189583547756*fd.sqrt(pi)*(-L/2-bink+xk)/(a_rad**3.0*fd.sqrt(1 + (-L/2-bink+xk)**2/a_rad**2))
        x1 = Z1
        KZ1 = 0.564189583547756*fd.sqrt(pi)*(L/2-bink+x1)/(a_rad**3.0*fd.sqrt(1+(L/2-bink+x1)**2/a_rad**2)) - 0.564189583547756*fd.sqrt(pi)*(-L/2-bink+x1)/(a_rad**3.0*fd.sqrt(1 + (-L/2-bink+x1)**2/a_rad**2))
        x0 = Z0
        KZ0 = 0.564189583547756*fd.sqrt(pi)*(L/2-bink+x0)/(a_rad**3.0*fd.sqrt(1+(L/2-bink+x0)**2/a_rad**2)) - 0.564189583547756*fd.sqrt(pi)*(-L/2-bink+x0)/(a_rad**3.0*fd.sqrt(1 + (-L/2-bink+x0)**2/a_rad**2))
        VPnl = (1/Lx)*( Mm*W12*(Z1-Z0)/dt - Mm*Z12*(W1-W0)/dt - Mm*W12*force12dt - 0.5*Mm*W12**2 - 0.5*kspring*(Z12-Zbar-force12)**2 + (Li*Ic12-ifac*KZ12)*(Q1-Q0)/dt - Q12*(Li*(I1-I0)-(KZ1-KZ0))/dt - 0.5*Li*Ic12**2 )*fd.dx(degree=vpolyp)
    ...
    Z_expr = fd.derivative(VPnl, W12, du=vvmpc2) # du=v_R eqn for Z1
    ...
    if noption==1:
        ...
        # W_expr = W_expr + (1/Lx)*( -vvmpc0*(gamma*GZZ12*Ic12 ) )*fd.dx(degree=vpolyp)
        W_expr = fd.replace(W_expr, {Q1: Q0+dt*Ic12}) # Q1 = Q0+dt*Ic12
...
while t <= 1.0*(t_end + dt): #
    ...
    if  nvpcase==2:
        ...
        force12dt.assign(dRwavemakerdt(t+0.5*dt,Amp,sigm,tstop))
        solve1coil_l.solve()
        Z12, W12, Ic12 = fd.split(result_mixedmpc)
