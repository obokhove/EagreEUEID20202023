...
if nvpcase == 0:
    ...
    eta_expr = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(eta_expr2,eta_new,bcs=BC_exclude_beyond_surface))
elif nvpcase==111: # clean version of 1112
    ...
    phi_combo = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr==0, result_mixed, bcs = [BC_exclude_beyond_surface_mixed,BC_varphi]))
    ...
    phi_combo1 = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr1==0, result_mixed, bcs = [BC_exclude_beyond_surface_mixed,BC_varphi]))
    # 
    # Step-3: f-derivative wrt phi but restrict to free surface to find updater eta_new; only solve for eta_new by using exclude
    eta_expr2 = fd.derivative(VP11, phii, du=v_W)
    eta_expr = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(eta_expr2,eta_new,bcs=BC_exclude_beyond_surface))
elif nvpcase==1112:
    # phif and eta are single space, phii, varphii splits of mixed space; all CG1 
    phii, varphii = fd.split(result_mixed)
    VP11 = ( fd.inner(phii, (eta_new - eta)/dt) + fd.inner(phi_f, eta/dt) - (1/2 * gg * fd.inner(eta, eta)) )* fd.ds(top_id) \
        - ( 1/2 * fd.inner(fd.grad(phii+varphii), fd.grad(phii+varphii))  ) * fd.dx
    # Step-1 and 2 must be solved in tandem: f-derivative VP wrt eta to find update of phi at free surface
