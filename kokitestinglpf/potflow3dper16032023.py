...
if nic=="SP1": # soliton decaying sufficiently fast to be taken as periodi using BL expansion in z
    ...
    X1KPE = k4**2*tau0-(1.0/k4)*np.arccosh( np.exp(k4**2*Y2KPE)-0.5*np.exp(-k4**2*Y2KPE) )
    X2KPE = k4**2*tau0+(1.0/k4)*np.arccosh( np.exp(k4**2*Y2KPE)-0.5*np.exp(-k4**2*Y2KPE) )
...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    xx11 = xx1 # 0.0
    xx22 = xx2 # Lx
    theta11 = -k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(xx1-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    theta41 = k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(xx1-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    ...
    KKXY1 = k4**4*(fd.exp(theta41)-fd.exp(theta11))
    ...
    theta12 = -k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(xx2-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    theta42 = k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(xx2-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    ...
    KKXY2 = k4**4*(fd.exp(theta42)-fd.exp(theta12))
    GX2 = 2*Fx**2*( k4**2*KKX2/KK2-3*KKXX2*KKX2/KK2**2+2*KKX2**3/KK2**3 ) # do it via fd .dx(0) derivative 2x ; use replace?
    GY2 = 2*Fy**2*( k4**2*KKXY2/KK2-2*KKXY2*KKY2/KK2**2-KKX2*KKYY2/KK2**2+2*KKX2*(KKY2)**2/KK2*2*3 ) # do it via fd .dx(1) derivative 2x
    ...
    theta1 = -k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(x[0]-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    theta4 =  k4*( (np.sqrt(eps)/H0)*(3/np.sqrt(2))**(1/3) )*(x[0]-0.5*Lx-np.sqrt(gg*H0)*t0) + k4**2*( (eps/H0)*(3/np.sqrt(2))**(2/3) )*(x[1]-0.5*Ly)
    ...
    KKXY = k4**4*(fd.exp(theta4)-fd.exp(theta1))
...
U0c0cor = fd.Function(V_R, name="U0c0cor") # topography fixed in time
...
if nvpcase=="MMP": # modfied midpoint for solving psimp, hmp, varphimp= fd.split(result_mixedmp)
    ...
    param_psi4555 = {'ksp_converged_reason':None, "ksp_rtol": 1e-8, 'fieldsplit_0_ksp_type': 'gmres', 'fieldsplit_0_pc_type': 'ilu',\
...
if nic=="SP2":
    print('Hallocorr?')
    outfile_corrsp2 = fd.File("results/corrsp2.pvd")
    outfile_corrsp2.write(U0c0cor, time=t)
