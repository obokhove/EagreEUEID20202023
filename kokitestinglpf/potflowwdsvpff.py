...
if nvpcase == 0:
    ax1.set_title(r'Weak form used:',fontsize=tsize2)
elif nvpcase == 1:
    ax1.set_title(r'Functional derivative VP used:',fontsize=tsize2)
elif nvpcase == 11:
    ax1.set_title(r'Functional derivative VP used, varphi and phif used, 3 steps:',fontsize=tsize2)
elif nvpcase == 111:
...
BC_exclude_beyond_surface_mixed = surface_BC_mixed()   # ONNO 30-12 do not understand what is inside: surface_BC() or MyBC() magic; no idea where explanation is
...
if nvpcase == 0:
...
elif nvpcase==11: # ONNO 19-12: 3 steps using varphi and phif
    VP = ( fd.inner(phi, (eta_new - eta)/dt) + fd.inner(phi_f, eta/dt) - (1/2 * gg * fd.inner(eta, eta)) )* fd.ds(top_id) \
        - ( 1/2 * fd.inner(fd.grad(phi), fd.grad(phi))  ) * fd.dx
    ...
    VP111 = ( fd.inner(phi, (eta_new - eta)/dt) + fd.inner(phi_f, eta/dt) - (1/2 * gg * fd.inner(eta, eta)) )* fd.ds(top_id) \
        - ( 1/2 * fd.inner(fd.grad(phi+varphi), fd.grad(phi+varphi))  ) * fd.dx
    # Step-1 and 2 must be solved in tandem: f-derivative VP wrt eta to find update of phi at free surface
    ...
    phif_expr11 = fd.derivative(VP111, eta, du=vvp0)  # du=v_W represents perturbation # 23-12 Make du split variable
    ...
    phi_expr11 = fd.derivative(VP111, varphi, du=vvp1) 
    phif_expr11 = fd.replace(phif_expr11, {phi: phii})  # replace if needed.
    phif_expr11 = fd.replace(phif_expr11, {varphi: varphii})  # replace if needed.
    phi_expr11 = fd.replace(phi_expr11, {phi: phii})  # replace if needed.
    phi_expr11 = fd.replace(phi_expr11, {varphi: varphii})  # replace if needed.
    Fexpr1 = phif_expr11+phi_expr11
    phi_combo1 = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(Fexpr1, result_mixed, bcs = [BC_exclude_beyond_surface_mixed,BC_varphi_mixed]))
    # 
    # Step-3: f-derivative wrt phi but restrict to free surface to find updater eta_new; only solve for eta_new by using exclude
    eta_expr2 = fd.derivative(VP11, phii, du=v_W)
    eta_expr22 = fd.derivative(VP111, phi, du=v_W)
    eta_expr = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(eta_expr2,eta_new,bcs=BC_exclude_beyond_surface))
elif nvpcase==2: # ONNO Steps 1 and 2 need solving in unison
    # 
    ...
    VPnl = ( H0*Ww*fd.inner(phii, (eta_new - eta)/dt) + H0*Ww*fd.inner(phi_f, eta/dt) - gg*Ww*H0*(0.5*fd.inner(H0+eta, H0+eta)-(H0+eta)*H0+0.5*H0**2) )* fd.ds(top_id) \
        - 1/2 * ( (Lw**2/Ww) * (H0+fac*eta) * (phii.dx(0)+varphii.dx(0)-(z/(H0+fac*eta))*fac*eta.dx(0)*(facc*phii.dx(1)+varphii.dx(1)))**2 + Ww * (H0**2/(H0+fac*eta)) * (facc*phii.dx(1)+varphii.dx(1))**2 ) * fd.dx
    ...
    # phi_exprnl = fd.NonlinearVariationalSolver(fd.NonlinearVariationalProblem(phi_exprnl1, varphi, bcs = BC_varphi))
...
while t <= t_end + epsmeet:
    ...
    if nvpcase == 0:
        ...
        # ONNO 27-12 EKin = fd.assemble( 0.5*fd.inner(fd.grad(phi+varphi),fd.grad(phi+varphi))*fd.dx )
        EKin = fd.assemble( 0.5*fd.inner(fd.grad(phii+varphii),fd.grad(phii+varphii))*fd.dx )
        # ONNO 27-12 EPot = fd.assemble( 0.5*gg*fd.inner(eta,eta)*fd.ds(top_id) )
        EPot = fd.assemble( 0.5*gg*fd.inner(eta,eta)*fd.ds(top_id) )
    elif nvpcase == 2:
