...
if nvpcase==1112: # as 11 but steps 1 and 2 in combined solve; step 3 separately; wavemaker
    ...
    Fdzdxi3 = (H0+etamp)/H0
    FJacobian = (1.0-(Rwave+FtnWM*Wflaps)/Lw)*(1+etamp/H0) # (Fdxdxi*Fdzdxi3 - Fdxdxi3*Fdzdxi)
    ...
    if norfullgrav ==1:
        VPnl = ( -fd.inner(phimp, (eta_new - eta)/dt) + fd.inner(etamp,(phiii-phi_f)/dt) \
                 ...
                 + 0.5*(1.0/FJacobian)*( (Fdxdxi3**2+Fdzdxi3**2) * ( Fdpsidxi0*phihat+varphimp.dx(0) )**2  \
                                              ...
                                              + (Fdxdxi**2+Fdzdxi**2)*( Fpsi0*phihat.dx(1)+varphimp.dx(1) )**2 ) * fd.dx(degree=(vpoly)) \
                                              + gg*FJacobian*(Fz-H0)*fd.dx(degree=(vpoly,vpoly)) \
                                              + ( Fdzdxi3*( dRwavedt+FtnWM*dWflapdts ) *( Fpsi0*phihat+varphimp ) ) * fd.ds_v(1,degree=vpolyp)
    else: #  Works:
        VPnl = ( -fd.inner(phimp, (eta_new - eta)/dt) + fd.inner(etamp,(phiii-phi_f)/dt) \
                 ...
                 + 0.5 * (1.0/FJacobian)* ( (Fdxdxi3**2+Fdzdxi3**2) * ( Fdpsidxi0*phihat+varphimp.dx(0) )**2  \
                                        -2.0 * (Fdxdxi*Fdxdxi3+Fdzdxi*Fdzdxi3) * ( Fdpsidxi0*phihat+varphimp.dx(0) ) * ( Fpsi0*phihat.dx(1)+varphimp.dx(1) ) \
                                        + (Fdxdxi**2+Fdzdxi**2)*( Fpsi0*phihat.dx(1)+varphimp.dx(1) )**2 ) * fd.dx(degree=(vpoly)) \
                                        + ( Fdzdxi3*( dRwavedt+FtnWM*dWflapdts ) *( Fpsi0*phihat+varphimp ) + Fdxdxi3*gg*(0.5*Fz**2-H0*Fz+facgaga*0.5*H0**2) )*fd.ds_v(1,degree=vpolyp) \
                                        + Fdxdxi *gg*(0.5*Fz**2-H0*Fz+facgaga*0.5*H0**2) * fd.ds_b(degree=vpolyp) \
                                        + Fdxdxi3*gg*(0.5*Fz**2-H0*Fz+facgaga*0.5*H0**2) * fd.ds_v(2,degree=vpolyp)
...
if nvpcase == 111:
    ...
    EKin = fd.assemble( 0.5 * (1.0/FfJacobian) * ( (Ffdxdxi3**2+Ffdzdxi3**2)*(phii.dx(0))**2 \
                                                  ...
                                                  + (Ffdxdxi**2+Ffdzdxi**2)*(phii.dx(1))**2) * fd.dx(degree=(vpoly)) ) # Note ONNO 21-02-2023: incorrect if nonzero phi
    if norfullgrav ==1:
        EPot = fd.assemble( gg*FfJacobian*(Ffz-H0)*fd.dx(degree=(vpoly)) )
    else: #
        EPot = fd.assemble( gg*Ffdxdxi*( 0.5*fd.inner(Ffz,Ffz)-H0*Ffz+facgaga*0.5*H0**2) * fd.ds_t(degree=vpolyp) \
                            + ( Ffdzdxi3*( dRwavedt+FftnWM*dWflapdts ) *( Fpsi0*phihat+varphimp ) + Ffdxdxi3*gg*(0.5*Ffz**2-H0*Ffz+facgaga*0.5*H0**2) ) * fd.ds_v(1,degree=vpolyp) )
    EKin = EKin
    EPot = EPot
elif nvpcase == 234: 
    Rwave.assign((1-nowaveflap)*Rwavemaker(0.0,gam,sigm,tstop))
...
while t <= t_end + dt: #
    ...
    if nvpcase == 111: # VP linear steps 1 and 2 combined
        ...
        EKin = fd.assemble( 0.5 * (1.0/FfJacobian) * ( (Ffdxdxi3**2+Ffdzdxi3**2)*(Ffdpsidxi0*fphihat+varphimp.dx(0))**2 \
                                    ...
                                    + (Ffdxdxi**2+Ffdzdxi**2)*(Ffpsi0*fphihat.dx(1)+varphimp.dx(1))**2 ) * fd.dx(degree=(vpoly)) )
        # 
        if norfullgrav ==1:
            EPot = fd.assemble( gg*FfJacobian*(Ffz-H0) * fd.dx(degree=(vpoly)) )
        else:
