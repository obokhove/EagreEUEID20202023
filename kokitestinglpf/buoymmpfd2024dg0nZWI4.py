...
mixed_Vmpc = V_C*V_C*V_C*V_C
result_mixedmpc = fd.Function(mixed_Vmpc)
vvmpc = fd.TestFunction(mixed_Vmpc)
vvmpc3, vvmpc2, vvmpc0, vvmpc1 = fd.split(vvmpc) # These represent "blocks".
# Q12, Z12 = fd.split(result_mixedmpc) # Only variations needed
Q12, Z12, W12, Ic12 = fd.split(result_mixedmpc) # But for completeness these enter the VP
...
if nvpcase==2: # MMP 1-coil nonlinear case
    ...
    Kfac = 1.0
    bink = Zbar + alp*Hm
    KZ12 = gamma*((Z12-bink+L/2)/(a_rad**2+(bink-L/2-Z12)**2)**(1/2)-(Z12-bink-L/2)/(a_rad**2+(bink+L/2-Z12)**2)**(1/2))/a_rad**2
    KZ1 = gamma*((Z1-bink+L/2)/(a_rad**2+(bink-L/2-Z1)**2)**(1/2)-(Z1-bink-L/2)/(a_rad**2+(bink+L/2-Z1)**2)**(1/2))/a_rad**2
    KZ0 = gamma*((Z0-bink+L/2)/(a_rad**2+(bink-L/2-Z0)**2)**(1/2)-(Z0-bink-L/2)/(a_rad**2+(bink+L/2-Z0)**2)**(1/2))/a_rad**2
    
    VPnl = (1/Lx)*( Mm*W12*(Z1-Z0)/dt - Mm*Z12*(W1-W0)/dt - Mm*W12*force12dt - 0.5*Mm*W12**2 - 0.5*kspring*(Z12-Zbar-force12)**2 + (Li*Ic12-Kfac*KZ12)*(Q1-Q0)/dt - Q12*(Li*(I1-I0)-Kfac*(KZ1-KZ0))/dt - 0.5*Li*Ic12**2 )*fd.dx(degree=vpolyp)  # 
        # VPnl = (1/Lx)*( Mm*W12*(Z1-Z0)/dt - Mm*Z12*(W1-W0)/dt - Mm*W12*force12dt - 0.5*Mm*W12**2 - 0.5*kspring*(Z12-Zbar-force12)**2 + (Li*Ic12-KZ12)*(Q1-Q0)/dt - Q12*(Li*(I1-I0))/dt - 0.5*Li*Ic12**2 )*fd.dx(degree=vpolyp) # does work with SWITCHON

    Q_expr = fd.derivative(VPnl, Ic12, du=vvmpc1)
    ...
    W_expr = fd.derivative(VPnl, Z12, du=vvmpc2) # du=v_R eqn for W1
    # W_expr = W_expr + (1/Lx)*( -vvmpc0*(gamma*GZZ12*(Ic12) ) )*fd.dx(degree=vpolyp) # works SWITCHON or next line
    # W W_expr = W_expr + (1/Lx)*( -vvmpc0*(gamma*GZZ12*(Q1-Q0)/dt ) )*fd.dx(degree=vpolyp) # work SWITCHON
    ...
    # W I_expr = I_expr + (1/Lx)*( vvmpc1*gamma*GZZ12*((Z1-Z0)/dt) )*fd.dx(degree=vpolyp)  # works SWITCHON
    I_expr = fd.replace(I_expr, {Z1: 2*Z12-Z0})
    I_expr = fd.replace(I_expr, {I1: 2*Ic12-I0})
    
    # Damping and load terms on current equation added to weak form: 
    I_expr = I_expr - (1/Lx)*( vvmpc3*((Rc+Ri+lfac*Rl)*Ic12 + tfac*fd.conditional(AbsI<smallfac,cc*ass*Ic12*(1-0.5*ass*AbsI+(ass*AbsI)**2/3-0.25*(ass*AbsI)**3+0.2*(ass*AbsI)**4),(cc*Ic12*fd.ln(1+ass*AbsI)/AbsI))  ) ) *fd.dx(degree=vpolyp)

    Fexpr = Z_expr+W_expr+I_expr+Q_expr
...
Pgave0nl = 0.0*np.array(Z0.vector())
Pgmeanl = 0.0*np.array(Z0.vector())
...
while t <= 1.0*(t_end + dt): #
    ...
    if  nvpcase==2:
        ...
        force12dt.assign(dRwavemakerdt(t+0.5*dt,Amp,sigm,tstop))
        solve1coil_l.solve()
        Q12, Z12, W12, Ic12 = fd.split(result_mixedmpc)
    ...
    if nvpcase==2:
        ...
        Pgave = Pgmean/t
        Pgnl1 = np.abs(I11)*nq*Vt*np.log(1+np.abs(I11)/Isat)
        Pgmeanl = Pgmeanl + dt*Pgnl1
        Pgavenl = Pgmeanl/t
        ...
        ax5.plot([t-dt,t],[Pgave0nl,Pgavenl],'-y')
        Pgave0 = Pgave
        Pgave0nl = Pgavenl
        Z0.assign(Z1)
        W0.assign(W1)
