...
if nic == 0:
    ...
    dtt = np.minimum(Lx/nx,Lz/nz)/(np.pi*np.sqrt(gg*H0)) # i.e. dx/max(c0) with c0 =sqrt(g*H0)
    Nt = 500 # check with print statement below and adjust dt towards dtt vi Nt halving time step seems to half energy oscillations
    CFL = 0.25 # run at a) 0.125 and b) 0.5*0.125
    dt = CFL*Tperiod/Nt  # 0.005  # time step [s]
    print('dtt=',dtt, t_end/dtt,dt,2/omega)
    D = 0.0
...
if nvpcase==1112: # as 11 but steps 1 and 2 in combined solve; step 3 separately; wavemaker
    ...
    gam = 0.5 #
    gam = 0.05 # waveflap test
    gam = 0.5 # waveflap test
    gam = 1. # waveflap test
    ...
    Fpsi = phimp/(1.0-( Rwave+FtnWM*Wflaps )/Lw)
    Fdpsidxi = (phimp.dx(0)/(1.0-( Rwave+FtnWM*Wflaps )/Lw)+(phimp/((1.0-( Rwave+FtnWM*Wflaps )/Lw)**2))*etamp.dx(0)*x[1]*( dWflapdzs )/Lw*H0)
    Fdpsidxi3 = (phimp.dx(1)/(1.0-( Rwave+FtnWM*Wflaps )/Lw)+(phimp/((1.0-( Rwave+FtnWM*Wflaps )/Lw)**2))*(H0+etamp)*( dWflapdzs )/Lw*H0)
    #         
    # Midpoint:
    ...
    VPnl = ( -fd.inner(phimp, (eta_new - eta)/dt) + fd.inner(etamp,(phiii-phi_f)/dt) \
             ...
             + Fpsi*(1.0-x[0]/Lw)*( dRwavedt+FtnWM*dWflapdts )*etamp.dx(0) )  * fd.ds_t \
             + 0.5 * (1.0/FJacobian)* ( (Fdxdxi3**2+Fdzdxi3**2) * ( Fdpsidxi+varphimp.dx(0) )**2  \
                                    ...
                                    + ( Fdzdxi3*( dRwavedt+FtnWM*dWflapdts ) *(Fpsi+varphimp) + Fdxdxi3*gg*(0.5*Fz**2-H0*Fz) ) * fd.ds_v(1)
...
if nvpcase == 111:
    ...
    Ffz = x[1]*(H0+eta)/H0
    Ffdxdxi = (1.0-( Rwave+Ffz*Wflaps )/Lw)+(x[1]/H0)*( dWflapdzs )*(1-x[0]/Lw)*eta.dx(0)
    ...
    EKin = fd.assemble( 0.5 * (1.0/FfJacobian) * ( (Ffdxdxi3**2+Ffdzdxi3**2)*(phii.dx(0))**2 \
                                                  ...
                                                  + (Ffdxdxi**2+Ffdzdxi**2)*(phii.dx(1))**2) * fd.dx ) 
...
while t <= t_end + dt: #
    ...
    if nvpcase == 111: # VP linear steps 1 and 2 combined
        ...
        Ffdpsidxi3 =  phimp.dx(1)/(1.0-( Rwave+Ffz*Wflaps )/Lw)+phimp/((1.0-( Rwave+Ffz*Wflaps )/Lw)**2)*(H0+etamp)*( dWflapdzs )/Lw
        EKin = fd.assemble( 0.5 * (1.0/FfJacobian) * ( (Ffdxdxi3**2+Ffdzdxi3**2)*(Ffdpsidxi+varphimp.dx(0))**2 \
                                    -2.0*(Ffdxdxi*Ffdxdxi3+Ffdzdxi*Ffdzdxi3)*(Ffdpsidxi+varphimp.dx(0))*(Ffdpsidxi3+varphimp.dx(1)) \
                                    + (Ffdxdxi**2+Ffdzdxi**2)*(Ffdpsidxi3+varphimp.dx(1))**2 ) * fd.dx )
        EPot = fd.assemble( gg*Ffdxdxi*( 0.5*fd.inner(Ffz,Ffz)-H0*Ffz+0.5*H0**2) * fd.ds_t )
        EKin = EKin
        EPot = EPot
    elif nvpcase == 24:
