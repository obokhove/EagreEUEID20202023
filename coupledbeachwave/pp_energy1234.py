...
with open(file,'r') as f:
    time, E_dw, E_sw, s_w, sw1, sw2, sw3, sw4, sw5 = np.loadtxt(f, usecols=(0,1,2,3,4,5,6,7,8), unpack=True)
...
no = 0
#  data3  0 0.01 0.1 0.2 0.3 0.4
if no==0:
    ax2.plot(time, s_w,'c-', label="0")
    # ax2.plot(time, sw1,'k.', label="h=0.1")
    ax2.plot(time, sw2,'b--', label="h=1e-2")
    ax2.plot(time, sw3,'r:', label="h=1e-3")
    ax2.plot(time, sw4,'g-.', label="h=1e-4")
    ax2.plot(time, sw5,'b-.', label="h=1e-5")
else:
    # Assume time, s_w, sw1, sw2, sw3, sw4, sw5 are already defined
    all_series = [s_w, sw1, sw2, sw3, sw4, sw5]
    # Loop through each time index
    for i in range(len(time)):
        if i % 100 == 0:
            y_values = [series[i] for series in all_series]
            ax2.plot([time[i]] * len(y_values), y_values, 'k.', alpha=0.3)
            ax2.plot([time[i]] * len(y_values), y_values, 'k-', alpha=0.3)
            plt.pause(0.000001) 
