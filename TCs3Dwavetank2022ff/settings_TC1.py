...
def test_case():
    ...
    #
    save_path = 'data/'+scheme+'/TC1_dt/'
    # save_path = 'data/'+scheme+'/TC1_dt_half/'
...
def set_time(Lx, res_x, Ly, res_y, H0, n_z, Tw):
    ...
    dz = H0/n_z
    dx = Lx/Nx
    dt = (min(dx,dz)/pi)       # time step at (min(dx,dz)/pi) or 0.5 that, e.g. 'half' then dt_save=2*dt
    T0 = 0.0                     # Initial time
    t = T0                  # Temporal variable 
    dt_save = dt          # saving time step at dt or when 0.5 used, set 2*dt
